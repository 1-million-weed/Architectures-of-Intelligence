...
# get random gain and bias for the three populations
...
# create random encoders for the three populations
...
# find the decoders for A, B and C
...
# calculate decoders for B with the function BC
...
while t < 10.0: 
    ...
    for j, s in enumerate(run_neurons(total_C, v_C, ref_C)):
        if s:
    ...
    ideal_C.append(function_BC(output_B))
    outputs_C.append(output_C)

    # output
    if t % 0.5 <= dt:
        print(f"{t}\t {output_A}\t {output_B}\t {output_C}")
    t += dt
